  ...
  */
  ...
  show (editor, options) {
    if (atom.config.get('autocomplete-plus.suggestionListFollows') === 'Cursor') {
      this.showAtCursorPosition(editor, options)
      return this
    } else {
      let { prefix } = options
      ...
      this.showAtBeginningOfPrefix(editor, prefix, followRawPrefix)
      return this
    }
  }
  ...
  showAtBeginningOfPrefix (editor, prefix, followRawPrefix = false) {
    ...
    if (this.activeEditor === editor) {
      ...
      if (editor) {
        ...
        this.placeMarker(editor, marker, {type: 'overlay', item: this.suggestionListElement, position: 'tail', class: 'autocomplete-plus'})
        return this
      }
    }
  }
  ...
  showAtCursorPosition (editor) {
    ...
    if (editor.getLastCursor()) {
      marker = this.suggestionMarker = editor.getLastCursor().getMarker()
    }
    if (marker) {
      this.placeMarker(editor, marker, {type: 'overlay', item: this.suggestionListElement, class: 'autocomplete-plus'})
      return this
    }
  }

  placeMarker (editor, marker, markerOptions) {
  ...
  hide () {
    this.destroyOverlay()
    if (this.activeEditor !== null && this.bindings && this.bindings.dispose) {
      this.bindings.dispose()
    }
    this.activeEditor = null
    return this
